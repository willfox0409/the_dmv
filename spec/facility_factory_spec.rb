...
RSpec.describe FacilityFactory do 
  before(:each) do
    ...
    @ny_dmv_data = @service.ny_dmv_office_locations
  end
  ...
  describe '#create_facilities' do
    ...
    it 'creates facility objects from the NY DMV data' do
        facilities = @factory.create_facilities(@ny_dmv_data)
        ...
        expect(first_facility.phone).to eq("Phone not available")  
        expect(first_facility.hours).to eq("Mon: CLOSED - CLOSED")  
        expect(first_facility.address_id).to eq("ID not available")  
      end
   end
end
